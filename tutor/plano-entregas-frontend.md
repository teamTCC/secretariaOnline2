...
| Camada | Onde ler |
|--------|----------|
| O que o back realmente faz | `transaçõesBackend/README.md` + pastas `F0`…`F8` |
...
**UI cega a perfil.** Botões existem só se `_links` vier na resposta. Proibido `if (role === 'SECRETARIO')`.
...
| **F1 Aluno** | BFF dashboard, primeiro acesso, perfil, hub, motor de requests + anexos MinIO, formativas, estágio, TCC, presença QR/PIN, certificados, atendimentos | `/bff/dashboard/aluno`, `/me`, `/requests`, `/formatives`, `/events`, `/certificates` |
...
| HATEOAS | hook `useActions(resource)` em `shared/api` | lei do produto |
...
| Push | `expo-notifications` → `POST /me/push-token` | transversal 10.5 |
...
- `/nova-senha?token=` → `POST /auth/reset-password` (precisa e-mail/Mailhog).
...
- `/perfil` mínimo: `GET /me` + `PATCH /me` (nome social/telefone/email pessoal — o que o DTO permitir). Avatar MinIO pode ficar stretch.
...
| `/solicitacoes/:id` | `GET /requests/{id}`, `GET /{id}/events`, anexos, protocolo `GET /{id}/protocol` |
| Action bar | só `rel`s de `_links` → `POST /{id}/transitions` (aluno: resubmit / pedir revisão) |

**DynamicForm:** widgets mínimos nesta entrega: text, textarea, select, date, number, checkbox. Stretch: `entity-select` (disciplinas `GET /academico/cursos/{id}/disciplinas`) e `file-upload` (presign). Sem file-upload, tipos que exigem anexo ficam demonstráveis só com tipos sem `x-required-attachments` — ainda assim o motor está validado.
...
| Área | Rotas | APIs |
|------|-------|------|
| Hub | `/comunicacao` | `GET /communications`, marcar lido |
...
- Estatísticas `GET /reports/secretary`.
...
| E2 | `solicitacoes` Request + Attachment + types, `academico` disciplinas | F1.005 + guia workflow |
...
- Não altera o backend. Se faltar um `_link` ou um campo no BFF, abrir issue para o time de back; o front não contorna com `role ===`.
